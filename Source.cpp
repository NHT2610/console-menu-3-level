...
int fContinueSelectCommand()
{
	char check;
	cout << "Ban co muon tiep tuc [Y/n]: "; cin >> check;
...
int fRunMenu1(int& lenh)
{
	if (lenh == 1)
	{
		...
		while (command != 0)
		{
			...
			command = fContinueSelectCommand();
		}
	}
	if (lenh == 2)
	{
...
int fMenu2()
{
	system("cls");
	cout << "===== MENU.2 =====" << endl;
	cout << "1. Tim phan tu lon nhat, nho nhat, xep hang phan tu" << endl;
...
int fRunMenu2(int& lenh)
{
	if (lenh == 1)
	{
		...
		while (command != 4)
		{
			command = fSelectCommand(fChildMenuGiaTriPhanTu1D());
			lenh = fRunChildMenuGiaTriPhanTu1D(command);
		}
	}
	if (lenh == 2)
	{
...
int fChildMenuGiaTriPhanTu1D()
{
	system("cls");
	cout << "===== Tim phan tu lon nhat, nho nhat && Xep hang phan tu =====" << endl;
	cout << "1. Tim phan tu lon nhat, nho nhat" << endl;
	cout << "2. Xep hang phan tu" << endl;
	cout << "3. Quay ve Menu.2" << endl;
	cout << "4. Thoat" << endl;
	return 4;
}
int fRunChildMenuGiaTriPhanTu1D(int& command)
{
	if (command == 1)
	{
		int Command = 1;
	...
	if (command == 2)
	{
		...
		{
			system("cls");
			int a[200], n = 0, index[200], count = 0, rank[200];
			cout << "===== Xep Hang Phan Tu Mang Mot Chieu =====" << endl;
			fNhapMang1Chieu(a, n);
			fXuatMang1chieu(a, n);
			fXepHangPhanTuMang1D(a, n, index, count, rank);
			cout << "BANG XEP HANG CAC PHAN TU TRONG MANG" << endl;
			cout << "Thu Hang" << "\t" << "Gia Tri" << "\t\t" << "Vi Tri" << endl;
			cout << endl;
			for (int i = 0; i < count; i++)
			{
				cout << rank[i] << "\t\t" << a[index[i]] << "\t\t" << index[i] << endl;
			}
			command = fContinueSelectCommand();
		}
	}
...
void fXepHangPhanTuMang1D(int a[], int& n, int index[], int& count, int rank[])
{
	int b[200];
	//Sao chép mảng a[] qua mảng tạm b[]
	for (int i = 0; i < n; i++)
	{
		b[i] = a[i];
	}
	//Sắp xếp các phần tử mảng b[] giảm dần
	fSapXepMangGiamDan(b, n);
	int dem = 1; //Biến dùng để đếm số thứ hạng các phần tử trong mảng (các phần tử bằng nhau thì cùng thứ hạng)
	for (int i = 0; i < n - 1; i++)
	{
		if (b[i] != b[i + 1]) { dem = dem + 1; }
	}
	int k = 0; //Biến đánh dấu vị trí đầu tiên của một thứ hạng
	for (int i = 1; i <= dem; i++)
	{
		int giaTri = b[k], STT = 1; //Biến giaTri là giá trị của các phần tử cùng một thứ hạng, biến STT là số thứ tự
									//của từng phần tử trong một thứ hạng 
		for (int j = k; j < n; j++)
		{
			rank[count] = i; //Lưu thứ hạng của phần tử
			fQuyDoiViTriMang1D(a, n, giaTri, STT, index, count);
			STT = STT + 1;
			if (b[j] != giaTri)
			{
				k = j;
...
void fQuyDoiViTriMang1D(int a[], int& n, int giaTri, int STT, int index[], int& count)
{
	int check = 1; //Biến đếm, kiểm tra để lưu đúng thứ tự của phần tử
	for (int i = 0; i < n; i++)
	{
		if (a[i] == giaTri)
		{
			if (check == STT) //Nếu check bằng STT, thì thực hiện lưu vị trí của phần tử a[i] vào index[]
			{
				index[count] = i;
				count += 1;
				check = check + 1; //Tăng biến check lên sau khi đã lưu đc giá trị, để các vòng lặp sau check!=STT
			}
			else { check = check + 1; } //Tăng biến đếm để check bằng STT
		}
	}
}
...
int fMenu3()
{
	...
	cout << "2. Phan tu lon nhat, nho nhat, xep hang phan tu" << endl;
...
int fRunMenu3(int& lenh)
{
	...
	if (lenh == 2)
	{
		...
		while (command != 4)
		{
			command = fSelectCommand(fChildMenuGiaTriPhanTu2D());
			lenh = fRunChildMenuGiaTriPhanTu2D(command);
		}
	}
	if (lenh == 3)
	{
...
int fChildMenuGiaTriPhanTu2D()
{
	system("cls");
	cout << "===== Phan tu lon nhat, nho nhat, xep hang phan tu =====" << endl;
	cout << "1. Phan tu lon nhat, nho nhat" << endl;
	cout << "2. Xep hang phan tu" << endl;
	cout << "3. Quay ve Menu.3" << endl;
	cout << "4. Thoat" << endl;
	return 4;
}
int fRunChildMenuGiaTriPhanTu2D(int& command)
{
	if (command == 1)
	{
		int Command = 1;
	...
	if (command == 2)
	{
		...
		{
			system("cls");
			cout << "===== Xep Hang Phan Tu Mang Hai Chieu =====" << endl;
			int a[10][10], m = 0, n = 0, count = 0, i_index[100], j_index[100], rank[100];
			fNhapMang2Chieu(a, m, n);
			fXuatMang2Chieu(a, m, n);
			fXepHangPhanTuMang2D(a, m, n, i_index, j_index, count, rank);
			cout << "BANG XEP HANG CAC PHAN TU TRONG MANG" << endl;
			cout << "Thu Hang" << "\t" << "Gia Tri" << "\t\t" << "Vi Tri" << endl;
			cout << endl;
			for (int i = 0; i < count; i++)
			{
				cout << rank[i] << "\t\t" << a[i_index[i]][j_index[i]] << "\t\t" << i_index[i] << "x" << j_index[i] << endl;
			}
			Command = fContinueSelectCommand();
		}
	}
...
void fXepHangPhanTuMang2D(int a[10][10], int& m, int& n, int i_index[], int j_index[], int& count, int rank[])
{
	int x[200], count2to1 = 0; //Mảng 1D x[] dùng để chuyển mảng a[10][10] sang mảng một chiều x[]
	//Chuyển mảng hai chiều a[10][10] về mảng một chiều x[]
	fChuyenMang2DThanh1D(a, m, n, x, count2to1);
	//Sắp xếp mảng x[] giảm dần
	fSapXepMangGiamDan(x, count2to1);
	//Đếm số lượng thứ hạng các phần tử trong mảng (các phần tử có giá trị bằng nhau thì cùng một thứ hạng)
	int dem = 1;
	for (int i = 0; i < count2to1 - 1; i++)
	{
		if (x[i] != x[i + 1]) { dem = dem + 1; }
	}
	int k = 0; //Biến k dùng để đánh dấu vị trí của phần tử đầu tiên trong một thứ hạng
	for (int i = 1; i <= dem; i++)
	{
		int giaTri = x[k]; //Lưu giá trị của một thứ hạng
		int STT = 1; //Sau mỗi vòng lặp i biến STT sẽ trở lại 1 để đánh dấu lại thứ tự cho thứ hạng tiếp theo
		for (int j = k; j < count2to1; j++)
		{
			rank[count] = i;
			fQuyDoiViTriMang2D(a, m, n, giaTri, STT, i_index, j_index, count);
			STT = STT + 1;
			if (x[j] != giaTri)
			{
				k = j;
...
void fQuyDoiViTriMang2D(int a[10][10], int& m, int& n, int giaTri, int STT, int i_index[], int j_index[], int& count)
{
	int check = 1; //Biến check để kiểm tra phần tử có đúng thứ tự trong thứ hạng 
	for (int i = 0; i < m; i++)
	{
		for (int j = 0; j < n; j++)
		{
			if (a[i][j] == giaTri)
			{
				if (check == STT)
				{
					...
					j_index[count] = j;
					count += 1;
					check = check + 1; //Tăng biến check sau khi đã tìm được đúng phần tử để vòng lặp sau check!=STT
				}
				else { check = check + 1; } //Tăng biến check cho đến khi check bằng STT
			}
		}
	}
}
